...
exports.generateTicket = async (req, res) => {
  try {
    ...
    // HEADER BAR
    const headerH = 80;
    doc.rect(0, 0, doc.page.width, headerH).fill(C.primary);
    doc.font('Helvetica-Bold').fontSize(22).fillColor(C.white).text('LETS GO SKY', 50, 30);
    doc.font('Helvetica').fontSize(10).fillColor(C.accent).text('Your journey begins with us', 50, 54);
    doc.fontSize(12).fillColor(C.white)
      .text('BOARDING PASS', doc.page.width - 140, 30, { align: 'right' })
      .text(`PNR: ${booking.pnrNumber}`, doc.page.width - 140, 50, { align: 'right' });

    // Main card
    const cardY = headerH + 20;
    const cardW = doc.page.width - 80;
    doc.roundedRect(40, cardY, cardW, 260, 8).fill(C.white).stroke(C.border);

    // Departure / Arrival codes
    const codeFont = 'Helvetica-Bold';
    const codeSize = 28;
    const codeY = cardY + 20;
    const leftX = 60;
    const rightX = 40 + cardW - 60;
    doc.font(codeFont).fontSize(codeSize).fillColor(C.primary)
      .text(booking.flight.departureAirportCode, leftX, codeY, { width: 100 });
    doc.text(booking.flight.arrivalAirportCode, rightX - doc.widthOfString(booking.flight.arrivalAirportCode), codeY);

    // Center arrow
    doc.font('Helvetica').fontSize(16).fillColor(C.light)
      .text('➝', 0, codeY + 6, { align: 'center' });

    // City names
    doc.font('Helvetica').fontSize(10).fillColor(C.light)
      .text(booking.flight.departureCity, leftX, codeY + 32)
      .text(booking.flight.arrivalCity, rightX - doc.widthOfString(booking.flight.arrivalCity), codeY + 32);

    // Flight details box
    const detailY = codeY + 70;
    const boxW = (cardW - 120) / 2;
    doc.roundedRect(60, detailY, boxW, 60, 5).fill(C.accent);
    doc.fontSize(12).fillColor(C.primary).text('DEPARTURE', 70, detailY + 8);
    doc.font('Helvetica-Bold').fontSize(14).fillColor(C.text).text(fmtDate(booking.flight.departureTime), 70, detailY + 25);
    doc.text(fmtTime(booking.flight.departureTime), 70, detailY + 43);

    doc.roundedRect(80 + boxW, detailY, boxW, 60, 5).fill(C.accent);
    doc.font('Helvetica').fontSize(12).fillColor(C.primary).text('FLIGHT', 90 + boxW, detailY + 8);
    doc.font('Helvetica-Bold').fontSize(14).fillColor(C.text)
      .text(`${booking.flight.airline}`, 90 + boxW, detailY + 25)
      .text(`${booking.flight.flightNumber}`, 90 + boxW, detailY + 43);

    // Passenger table
    const passY = detailY + 90;
    const rowH = 20;
    doc.font('Helvetica-Bold').fontSize(12).fillColor(C.primary)
      .text('PASSENGERS', 60, passY);
    booking.passengers.forEach((p, i) => {
      const y = passY + (i + 1) * rowH;
      doc.font('Helvetica').fontSize(10).fillColor(C.text)
        .text(p.name, 60, y)
        .text(p.age.toString(), 260, y)
        .text(p.gender, 320, y)
        .text(p.seatNumber || 'TBD', 400, y);
    });

    // Payment box
    const payY = passY + (booking.passengers.length + 1) * rowH + 20;
    doc.roundedRect(40, payY, cardW, 70, 8).fill(C.primary);
    doc.font('Helvetica-Bold').fontSize(12).fillColor(C.white).text('PAYMENT', 60, payY + 12);
    const amt = `₹${booking.totalPrice.toFixed(2)}`;
    doc.fontSize(14).text(amt, 60, payY + 30);
    doc.font('Helvetica').fontSize(10).text(`Status: ${booking.status}`, 300, payY + 30);

    // Footer info
    const footY = payY + 90;
    doc.font('Helvetica').fontSize(9).fillColor(C.light)
      .text('Arrive 2 hrs before departure | Carry valid ID | Non-transferable | Web check-in opens 48hrs prior', 40, footY, { width: cardW });

    doc.end();
    stream.on('finish', () => {
      res.download(filePath, `LetsGoSky_${booking.pnrNumber}.pdf`, err => { if (err) res.status(500).json({ message: 'Error sending file' }); fs.unlinkSync(filePath); });
    });
  } catch (err) {
    console.error(err);
